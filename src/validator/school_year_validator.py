...
def validate_school_year(df, config, cross_data=None):
    ...
    if rules.get("state_reporting_start_check", True):
        ...
        if col in df.columns:
            # Convert to datetime (handles strings or Excel-like dates)
            parsed_dates = pd.to_datetime(df[col], errors="coerce")

            # Extract MM-DD and compare
            invalid_mask = ~parsed_dates.dt.strftime("%m-%d").eq("07-01")

            add_error(invalid_mask, f"{col} must have MM-DD of 07-01")
    ...
    if rules.get("start_year_match_check", True):
        ...
        if date_col in df.columns and year_col in df.columns:
            ...
            # Use helper to extract the start year
    ...
    if rules.get("last_day_check", True):
        ...
        if col in df.columns:
            ...
            # Flag rows where the MM-DD is not 06-30
            invalid_mask = ~parsed_dates.dt.strftime("%m-%d").eq("06-30")

            add_error(invalid_mask, f"{col} must have MM-DD of 06-30")
    ...
    if rules.get("school_year_format_check", True):
        ...
        if year_col in df.columns:
            # Match exact format: four digits, optional space, dash, optional space, four digits
            valid_format_mask = df[year_col].astype(str).str.match(r"^\d{4}\s*-\s*\d{4}$")

            # Flag rows that don't match
            invalid_mask = ~valid_format_mask
    ...
    if rules.get("date_format_check", True):
        ...
        for col in date_fields:
            if col in df.columns:
                # Ensure value parses to a date
                parsed_dates = pd.to_datetime(df[col], errors="coerce")
                original_series = df[col].astype(str).str.strip()

                # Build a regex to match strictly MM/DD/YYYY
                valid_format = original_series.str.match(r"^(0[1-9]|1[0-2])/([0][1-9]|[12]\d|3[01])/(\d{4})$")

                # Flag values that are either unparseable OR wrong format
                invalid_mask = parsed_dates.isna() | ~valid_format
    ...
    if rules.get("school_year_suffix_check", True):
        # List of (start_col, end_col, label)
        date_pairs = [
    ...
    if rules.get("date_order_check", True):
        ...
        for start_col, end_col, label in date_pairs:
            if start_col in df.columns and end_col in df.columns:
                ...
                # Flag if start is not before end or dates are invalid
                invalid_mask = (
                    parsed_start.isna() |
                    parsed_end.isna() |
